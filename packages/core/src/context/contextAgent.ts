...
export class ContextAgent {
   ...
   * Milestone 3: Enhanced context injection with smart analysis
   */
  async getContextForPrompt(userInput?: string): Promise<string> {
    ...
    if (this.config.getDebugMode()) {
      console.log('[ContextAgent] getContextForPrompt called (Milestone 3: enhanced context injection)');
    }
    
    try {
      // Get knowledge graph statistics for basic context
      const stats = this.knowledgeGraph.getStatistics();
      if (!stats || stats.totalNodes === 0) {
        return '';
      }

      // Generate enhanced project context
      const contextSections: string[] = [];
      
      contextSections.push('# 🧠 AI-Enhanced Project Context');
      contextSections.push('*Intelligently generated by ContextAgent using static code analysis*');
      contextSections.push('');
      
      // Project architecture overview
      contextSections.push('## 🏗️ Project Architecture');
      contextSections.push(`- **Code files**: ${stats.fileNodes} analyzed`);
      contextSections.push(`- **Functions/methods**: ${stats.functionNodes} discovered`);
      contextSections.push(`- **Classes/types**: ${stats.classNodes} identified`);
      contextSections.push(`- **External dependencies**: ${stats.moduleNodes} modules`);
      contextSections.push(`- **Relationships**: ${stats.importRelations} imports, ${stats.callRelations} function calls`);
      contextSections.push('');

      // Add project structure insights
      const structureInsights = await this.getProjectStructureInsights();
      if (structureInsights.length > 0) {
        contextSections.push('## 📁 Project Structure Insights');
        contextSections.push(...structureInsights);
        contextSections.push('');
      }

      // User input specific context
      if (userInput) {
        const relevantContext = await this.findRelevantContext(userInput);
        if (relevantContext.length > 0) {
          contextSections.push('## 🎯 Query-Relevant Context');
          contextSections.push('*Code elements related to your request:*');
          contextSections.push('');
        ...
        const intentSuggestions = this.analyzeUserIntent(userInput);
        if (intentSuggestions.length > 0) {
          contextSections.push('## 💡 Contextual Suggestions');
          contextSections.push(...intentSuggestions);
          contextSections.push('');
        }
      }

      // Add recent changes context if available
      const recentChanges = await this.getRecentChangesContext();
      if (recentChanges.length > 0) {
        contextSections.push('## 🔄 Recent Changes');
        contextSections.push('*Files recently modified in your project:*');
      ...
      if (this.config.getDebugMode()) {
        console.log(`[ContextAgent] Generated enhanced context: ${context.length} characters`);
        console.log(`[ContextAgent] Context sections: ${contextSections.filter(s => s.startsWith('##')).length}`);
      }
      
      return context;
      
    } catch (error) {
      console.error('[ContextAgent] Failed to generate context:', error);
      return '';
    }
  }
