...
export class FullJsonParser {
  ...
  parseResponse(content: string): FullJsonResponse {
    ...
    try {
      ...
      if (jsonBlocks.length > 0) {
        ...
        for (let i = 0; i < jsonBlocks.length; i++) {
          try {
            ...
            console.log(`❌ [FullJsonParser] JSON块 ${i+1} 解析失败: ${blockError}`);
            console.log(`📄 [FullJsonParser] 问题块内容: ${jsonBlocks[i].slice(0, 100)}...`);
            continue;
          }
        }
      }
      ...
      console.log(`📄 [FullJsonParser] 失败内容末尾: ...${content.slice(-100)}`);
  ...
  private extractJsonBlocks(content: string): string[] {
    ...
    while ((match = jsonPattern.exec(content)) !== null) {
      console.log(`🔍 [FullJsonParser] 找到代码块JSON: ${match[1].slice(0, 50)}...`);
    ...
    if (jsonBlocks.length === 0) {
      ...
      if (standaloneMatch) {
        console.log(`🔍 [FullJsonParser] 找到独立JSON: ${standaloneMatch[1].slice(0, 50)}...`);
    ...
    if (jsonBlocks.length === 0) {
      ...
      if (firstBrace !== -1 && lastBrace !== -1 && lastBrace > firstBrace) {
        ...
        console.log(`🔍 [FullJsonParser] 找到可能的JSON: ${possibleJson.slice(0, 50)}...`);
   ...
   * 规范化JSON数据到标准格式
   */
  private normalizeJsonData(jsonData: any): FullJsonResponse {
    ...
    // 提取思考/推理过程
    result.thinking = jsonData.thinking || jsonData.reasoning || jsonData.analysis;
    ...
    if (jsonData.tool_calls && Array.isArray(jsonData.tool_calls)) {
      result.tool_calls = jsonData.tool_calls;
    } else if (jsonData.tools && Array.isArray(jsonData.tools)) {
      result.tool_calls = jsonData.tools;
    } else if (jsonData.function_calls && Array.isArray(jsonData.function_calls)) {
      result.tool_calls = jsonData.function_calls;
    }

    console.log(`✅ [FullJsonParser] 解析完成: 消息=${!!result.message}, 工具=${result.tool_calls.length}个`);
  ...
  private attemptJsonFix(content: string): string | null {
    ...
    // 修复5: 缺失的逗号在工具调用之间
    fixed = fixed.replace(/"tool":\s*"([^"]+)"\s+("args")/g, '"tool": "$1",\n      $2');
    ...
    if (!fixed.endsWith('}') && !fixed.endsWith(']')) {
      ...
      console.log(`🔧 [FullJsonParser] 补全了 ${Math.max(0, braceCount)} 个大括号和 ${Math.max(0, bracketCount)} 个方括号`);
    }
   ...
   */
  convertToFunctionCalls(toolCalls: Array<{tool: string, args: Record<string, any>}>): Array<ConvertedFunctionCall> {
    console.log(`🔄 [FullJsonParser] 转换 ${toolCalls.length} 个工具调用为Function Calls`);
    ...
    return toolCalls.map((toolCall, index) => {
      ...
      console.log(`   🔧 [FullJsonParser] ${toolCall.tool}(${JSON.stringify(toolCall.args)}) → ${callId}`);
   ...
   * 获取工具调用统计信息
   */
  getToolCallStats(parsedResponse: FullJsonResponse): {count: number, tools: string[]} {
  ...
  private extractPartialJson(content: string): string | null {
    ...
    if (lastValidPos > 0) {
      ...
      console.log(`🔧 [FullJsonParser] 找到部分JSON，长度: ${partialJson.length}`);
      return partialJson;
    }
