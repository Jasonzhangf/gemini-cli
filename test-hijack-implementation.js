...
async function testHijackImplementation() {
  ...
  try {
    ...
    const contentGeneratorPath = path.join('/Users/fanzhang/Documents/github/gemini-cli/packages/core/dist/core/contentGenerator.js');
    ...
    if (fs.existsSync(contentGeneratorPath)) {
      ...
      console.log('🔄 Expected behavior: Model should be hijacked to blacktooth-ab-test via http://127.0.0.1:2048/v1');
    } else {
      console.log('❌ ContentGenerator module not found. Please run npm run build first.');
    }
